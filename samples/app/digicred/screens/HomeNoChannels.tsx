...
const HomeNoChannels = () => {
  ...
  useEffect(() => {
    if (!loading && !hasChecked.current) {
      ...
      const hasConnection = connections.some((conn) => {
        ...
        if (!store.preferences.developerModeEnabled && conn.state !== DidExchangeState.Completed) {
          return false
        }
        return true
      })
    ...
    if (!loading) {
      Animated.parallel([
        ...
        Animated.stagger(200, [
          ...
          Animated.timing(item2Anim, { toValue: 1, duration: 400, delay: 200, useNativeDriver: true }),
          Animated.timing(item3Anim, { toValue: 1, duration: 400, delay: 400, useNativeDriver: true }),
        ]).start()
      })
    }
  }, [fadeAnim, item1Anim, item2Anim, item3Anim, loading, translateYAnim])
  ...
  const circleSize = Math.max(SCREEN_WIDTH, SCREEN_HEIGHT) * 0.9
      ...
      <SafeAreaView style={styles.container}>
        <View
          style={[
            styles.darkCircle,
            {
              ...
              top: -circleSize * (Platform.OS === 'ios' ? 0.5 : 0.5),
            },
          ]}
        />
        <View style={styles.scrollContent}>
          ...
          >
            ...
            <Text style={styles.description}>{t('homeNoChannels.description')}</Text>
            <LinearGradient
            ...
            <Text style={styles.howItWorks}>{t('homeNoChannels.howItWorks')}</Text>
            <Animated.View
              style={[
                styles.animatedItem,
                {
            ...
            >
              <TouchableOpacity style={styles.item} onPress={() => handleItemPress(0)} activeOpacity={0.7}>
            ...
            >
              <TouchableOpacity style={styles.item} onPress={() => handleItemPress(1)} activeOpacity={0.7}>
                <View style={styles.iconWrapper}>
            ...
            >
              <TouchableOpacity style={styles.item} onPress={() => handleItemPress(2)} activeOpacity={0.7}>
                <View style={styles.iconWrapper}>
